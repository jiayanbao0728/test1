...
autoSendSlider.addEventListener('change', () => {
    ...
    const openEnvelopePanel = async (shouldHideAdvanced = false) => {
        if (shouldHideAdvanced) hideModal(DOMElements.advancedModal.modal);
    ...
    if (envelopeEntryBtn) {
        envelopeEntryBtn.addEventListener('click', () => openEnvelopePanel(true));
    }
    const headerEnvelopeBtn = document.getElementById('header-envelope-btn');
    if (headerEnvelopeBtn) {
        headerEnvelopeBtn.addEventListener('click', () => openEnvelopePanel(false));
    }
