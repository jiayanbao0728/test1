...
(function() {
    ...
    function openStatsView() {
        ...
        if (modal) modal.classList.add('cd-stats-open');
        updateStatsToggleLabel(true);
        renderStats();
    }
    function closeStatsView() {
        ...
        if (modal) modal.classList.remove('cd-stats-open');
        updateStatsToggleLabel(false);
    }
    function updateStatsToggleLabel(isOpen) {
        document.querySelectorAll('#cd-stats-btn, .cd-toolbar-stats-btn').forEach(btn => {
            const icon = btn.querySelector('i');
            const label = btn.querySelector('span');
            if (icon) {
                icon.classList.toggle('fa-chart-pie', !isOpen);
                icon.classList.toggle('fa-arrow-left', isOpen);
            }
            if (label) label.textContent = isOpen ? '\u8fd4\u56de' : '\u7edf\u8ba1';
        });
    }
    function toggleStatsView() {
        const view = document.getElementById('cd-stats-view');
        if (view && view.classList.contains('open')) closeStatsView();
        else openStatsView();
    }
    ...
    function bindEvents() {
        ...
        if (statsBtn && !statsBtn.dataset.cdBound) {
            ...
            statsBtn.addEventListener('click', toggleStatsView);
        }
