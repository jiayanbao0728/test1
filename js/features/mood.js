...
function getMoodOptionByKey(key) {
    return getAllMoodOptions().find(mood => mood.key === key);
}

function formatMoodCompanionDuration(seconds) {
...
function syncMoodRecordToChat(dateStr, moodKey, target) {
    if (!moodKey) return;
    const moodObj = getMoodOptionByKey(moodKey);
    if (!moodObj) return;
    const todayStr = formatDateStr(new Date());
    const datePrefix = dateStr === todayStr ? '今天' : dateStr;
    ...
    const subject = target === 'partner' ? `${partnerName}的心情` : `${myName}的心情`;
    if (typeof window._addCallEvent === 'function') {
        window._addCallEvent('fa-face-smile', `${datePrefix}记录了${subject}：${moodObj.label}`, moodObj.kaomoji);
    }
}

window.addCompanionRecordToMood = function(record) {
    try {
        if (!record || !record.modeLabel) return;
        const startDate = record.startedAt ? new Date(record.startedAt) : new Date();
        const dateStr = formatDateStr(startDate);
        if (!moodData[dateStr]) moodData[dateStr] = {};
        if (!Array.isArray(moodData[dateStr].companions)) moodData[dateStr].companions = [];

        const entry = {
            id: record.id || `${dateStr}-${record.mode || 'companion'}-${record.startedAt || Date.now()}-${Math.round(record.duration || 0)}`,
            mode: record.mode || '',
            label: record.modeLabel,
            duration: Math.max(0, Math.floor(Number(record.duration) || 0)),
            startedAt: record.startedAt || Date.now(),
            initiator: record.initiator || 'user'
        };

        if (moodData[dateStr].companions.some(item => item.id === entry.id)) return;
        moodData[dateStr].companions.unshift(entry);
        moodData[dateStr].companions = moodData[dateStr].companions.slice(0, 20);
        saveMoodData();
    } catch (e) {
        console.warn('[mood] add companion record error:', e);
    }
};
...
function renderMoodCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (dayData) {
            ...
            if (Array.isArray(dayData.companions) && dayData.companions.length) {
                const dot = document.createElement('div');
                dot.className = 'mood-detail-dot';
                dot.title = '陪伴记录';
                dot.style.backgroundColor = 'var(--accent-color)';
                dot.style.boxShadow = '0 0 0 2px rgba(197,164,126,0.18)';
                dotsContainer.appendChild(dot);
            }
        }
        ...
        dayDiv.addEventListener('click', () => {
            ...
            if (dayEntry && (dayEntry.user || dayEntry.partner || (Array.isArray(dayEntry.companions) && dayEntry.companions.length))) {
...
window.deleteDailyMood = function(dateStr, who) {
    ...
    if (
        !moodData[dateStr].user &&
        !moodData[dateStr].partner &&
        (!Array.isArray(moodData[dateStr].companions) || moodData[dateStr].companions.length === 0)
    ) {
...
document.getElementById('confirm-mood-save').addEventListener('click', () => {
    ...
    saveMoodData();
    syncMoodRecordToChat(
        selectedDateStr,
        currentMoodEditTarget === 'me' ? moodData[selectedDateStr].user : moodData[selectedDateStr].partner,
        currentMoodEditTarget
    );
    closeMoodOverlay();
...
function showDayDetails(dateStr, data) {
    ...
    const companionSection = document.getElementById('detail-companion-section');
    const companionList = document.getElementById('detail-companion-list');
    if (companionSection && companionList) {
        const companions = Array.isArray(data.companions) ? data.companions : [];
        if (companions.length) {
            companionSection.style.display = 'block';
            companionList.innerHTML = companions.map(item => `
                <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; padding:7px 9px; border-radius:10px; background:var(--secondary-bg); color:var(--text-primary); font-size:12px;">
                    <span>${item.label || '陪伴'}</span>
                    <span style="color:var(--accent-color); font-weight:600;">${formatMoodCompanionDuration(item.duration)}</span>
                </div>
            `).join('');
        } else {
...
function initMoodListeners() {
    ...
    const headerMoodBtn = document.getElementById('header-mood-btn');
    const modal = document.getElementById('mood-modal');
    const openMoodPanel = (shouldHideAdvanced) => {
        ...
        if (shouldHideAdvanced && advModal) hideModal(advModal);
        setTimeout(() => {
            renderMoodCalendar();
            showModal(modal);
        }, shouldHideAdvanced ? 150 : 0);
    };
    ...
    if (entryBtn && !entryBtn.dataset.initialized) {
        ...
        newBtn.addEventListener('click', () => openMoodPanel(true));
    }

    if (headerMoodBtn && !headerMoodBtn.dataset.initialized) {
        headerMoodBtn.dataset.initialized = 'true';
        headerMoodBtn.addEventListener('click', () => openMoodPanel(false));
    }
