...
(function () {
    ...
    const DEFAULT_TIME_OPTIONS = [10, 30, 60, 120, 'custom', 'rest'];
    ...
    const MODES = {
        study:    {
            ...
            icon:  'fa-book-open',
            hint:  '正在一起学习 · 加油',
            times: DEFAULT_TIME_OPTIONS,        // 用户可选
    ...
    function openCompanionModal() {
        ...
        modal.innerHTML = `
            <div id="companion-modal-card" style="
                ...
                background:var(--secondary-bg, #fff);border-radius:36px;padding:74px 28px 28px;
                width:min(92vw, var(--milk-dialog-width, 390px));
                max-width:var(--milk-dialog-width, 390px);
                height:var(--milk-dialog-height, min(90vh, 520px));
                max-height:min(90vh, var(--milk-dialog-height, 520px));
                overflow:hidden;display:flex;flex-direction:column;
                box-shadow:0 20px 60px rgba(0,0,0,0.18);
                opacity:1 !important;transform:none !important;
                animation:companionPopIn 0.3s cubic-bezier(0.34,1.56,0.64,1);
            ">
                <button id="companion-dynamic-back" type="button" style="
                    position:absolute;left:22px;top:22px;width:34px;height:34px;border-radius:13px;
                    border:1.5px solid var(--border-color, rgba(0,0,0,0.1));
                    background:var(--primary-bg, #f8f8f8);color:var(--text-secondary, #666);
                    display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;
                    font-family:var(--font-family, 'Noto Serif SC', serif);
                " aria-label="返回"><i class="fas fa-arrow-left"></i></button>
                <button id="companion-dynamic-close" type="button" style="
                    position:absolute;right:22px;top:22px;width:34px;height:34px;border-radius:13px;
                    border:1.5px solid transparent;
                    background:rgba(var(--accent-color-rgb,197,164,126),0.08);
                    color:var(--accent-color, #c5a47e);
                    display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;
                    font-family:var(--font-family, 'Noto Serif SC', serif);
                " aria-label="关闭"><i class="fas fa-times"></i></button>
                <div style="display:flex;align-items:center;gap:8px;font-size:20px;font-weight:500;color:var(--text-primary, #1a1a1a);margin-bottom:18px;justify-content:center;font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;flex:0 0 auto;">
                    <i class="fas fa-hand-holding-heart" style="color:var(--accent-color, #c5a47e);"></i>
                    <span>陪伴</span>
                </div>
                <div id="companion-cards-wrap" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:repeat(2,minmax(0,1fr));gap:14px;padding:2px 0;flex:1 1 auto;min-height:0;">
                    ${Object.entries(MODES).map(([key, cfg]) => `
                        <div class="companion-mode-card-dyn" data-mode="${key}" style="
                            background:var(--primary-bg, #fafafa);border-radius:18px;padding:14px 10px;cursor:pointer;
                        ...
                        ">
                            ...
                            <span style="font-size:14px;font-weight:500;color:var(--text-primary, #1a1a1a);font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">${cfg.label}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
        ...
        modal.querySelector('#companion-dynamic-back').addEventListener('click', () => {
            const returnTarget = window.__companionReturnTarget;
            window.__companionReturnTarget = '';
            closeCompanionModal();
            if (returnTarget === 'advanced-modal') {
                const advancedModal = document.getElementById('advanced-modal');
                if (advancedModal) {
                    if (typeof showModal === 'function') showModal(advancedModal);
                    else advancedModal.style.display = 'flex';
                }
            }
        });
    ...
    function showCompanionInviting(mode) {
        ...
        let userTimeText;
        if (!isCountdown) {
            userTimeText = getOpenEndedLabel(mode);
        } else {
            const minutes = Math.round(totalSeconds / 60);
            userTimeText = formatDurationText(minutes);
        }
        ...
        if (willReject) {
            ...
            window._invitingTimer = setTimeout(() => {
                ...
                const sceneName = MODES[mode]?.label?.replace(/^一起/, '') || '';
                // 像通话未接通一样，用中性的系统事件记录这次邀请结果
                sendChatEvent('fa-circle-xmark', `${sceneName}邀请未接通`, null);
                if (typeof showNotification === 'function') {
                    showNotification(`${partnerName} 暂时没有接起陪伴邀请`, 'info');
                }
                // 过渡画面：梦角原话
    ...
    function weightedPick(items) {
        ...
        if (total <= 0) return pickRandom(Object.keys(MODES));
        let roll = Math.random() * total;
        for (const item of items) {
            roll -= Math.max(0, item.weight || 0);
            if (roll <= 0) return item.mode;
        }
        return items[items.length - 1]?.mode || 'study';
    }

    function pickContextualInviteMode(now = new Date()) {
        ...
        if (hour >= 6 && hour < 9) {
            weights = [
                { mode: 'study', weight: 4 },
                { mode: 'work', weight: isWeekend ? 1 : 3 },
                { mode: 'exercise', weight: isWeekend ? 3 : 1 },
                { mode: 'sleep', weight: 0.2 }
            ];
        } else if (hour >= 9 && hour < 12) {
            weights = [
                { mode: 'study', weight: isWeekend ? 4 : 3 },
                { mode: 'work', weight: isWeekend ? 1 : 5 },
                { mode: 'exercise', weight: isWeekend ? 2 : 0.6 },
                { mode: 'sleep', weight: 0.1 }
            ];
        } else if (hour >= 12 && hour < 14) {
            weights = [
                { mode: 'study', weight: 1.5 },
                { mode: 'work', weight: isWeekend ? 0.6 : 1.5 },
                { mode: 'exercise', weight: 2 },
                { mode: 'sleep', weight: 1.2 }
            ];
        } else if (hour >= 14 && hour < 18) {
            weights = [
                { mode: 'study', weight: isWeekend ? 4 : 2.5 },
                { mode: 'work', weight: isWeekend ? 1 : 4 },
                { mode: 'exercise', weight: isWeekend ? 3 : 1.2 },
                { mode: 'sleep', weight: 0.1 }
            ];
        } else if (hour >= 18 && hour < 22) {
            weights = [
                { mode: 'study', weight: 2.5 },
                { mode: 'work', weight: isWeekend ? 0.5 : 1 },
                { mode: 'exercise', weight: 3 },
                { mode: 'sleep', weight: 1.4 }
            ];
        } else if (hour >= 22 || hour < 1) {
            weights = [
                { mode: 'study', weight: 0.5 },
                { mode: 'work', weight: 0.2 },
                { mode: 'exercise', weight: 0.1 },
                { mode: 'sleep', weight: 6 }
            ];
        } else {
            weights = [
                { mode: 'study', weight: 0.1 },
                { mode: 'work', weight: 0.1 },
                { mode: 'exercise', weight: 0 },
                { mode: 'sleep', weight: 7 }
            ];
        }

        return weightedPick(weights.filter(item => MODES[item.mode]));
    }
    ...
    async function showIncomingCompanion(mode) {
        ...
        // 如果没指定 mode，按当前时间段和周末权重选择一个更自然的场景
        if (!mode) {
            mode = pickContextualInviteMode();
        }
        currentMode = mode;
        const cfg = MODES[mode];
        const partnerName = getPartnerName();
        ...
        if (inviteTime === 'rest') {
            ...
            const timeText = formatDurationText(inviteTime);
            // 台词里已经有"陪你"/"陪着你" → 直接加时间
        ...
        const autoTimer = setTimeout(() => {
            ...
            overlay.remove();
            const sceneName = MODES[mode]?.label?.replace(/^一起/, '') || '';
            sendChatEvent('fa-circle-xmark', `未接起${partnerName}的${sceneName}邀请`, null);
        }, 22000);
        ...
        overlay.querySelector('#companion-incoming-reject').addEventListener('click', () => {
            ...
            const sceneName = MODES[mode]?.label?.replace(/^一起/, '') || '';
            sendChatEvent('fa-circle-xmark', `我没有接起这次${sceneName}邀请`, null);
            // 过渡画面：旁白·梦角第一人称
    ...
    function getOpenEndedLabel(mode) {
        return {
            study: '好好学习',
            work: '好好工作',
            exercise: '好好运动',
    ...
    function getOpenEndedIcon(mode) {
        return MODES[mode]?.icon || 'fa-hand-holding-heart';
    }

    function formatTimeOptionLabel(minutes) {
        if (minutes === 60) return { main: '1', unit: '小时' };
        if (minutes === 120) return { main: '2', unit: '小时' };
        return { main: String(minutes), unit: '分钟' };
    }
    ...
    function normalizeCustomMinutes(raw) {
        const minutes = Math.round(Number(String(raw).trim()));
        if (!Number.isFinite(minutes) || minutes <= 0) {
            notify('请输入有效的分钟数', 'warning');
            return null;
        }
        if (minutes > 1440) {
            notify('自定义时长不能超过 1440 分钟', 'warning');
            return null;
        }
        return minutes;
    }

    function askCustomMinutes() {
        const raw = window.prompt('请输入陪伴时长（分钟）', '45');
        if (raw === null) return null;
        return normalizeCustomMinutes(raw);
    }
    ...
    function applySelectedTime(value) {
        if (value === 'custom') {
            const customMinutes = askCustomMinutes();
            if (!customMinutes) return null;
            return setCountdownMinutes(customMinutes);
        }
        ...
        return setCountdownMinutes(minutes);
    }
    ...
    function openTimeModal(mode, onSelected) {
        ...
        const timesHtml = cfg.times.map(t => {
            if (t === 'rest') {
                ...
                ">
                    <i class="fas ${getOpenEndedIcon(mode)}" style="font-size:20px;color:var(--accent-color, #c5a47e);"></i>
                    <span style="font-size:13px;font-weight:500;color:var(--text-primary, #1a1a1a);font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">${getOpenEndedLabel(mode)}</span>
                </button>`;
            }
            if (t === 'custom') {
                return `<button class="time-btn-dyn" data-time="custom" style="
                    background:var(--secondary-bg, #fff);border:1.5px solid var(--border-color, #eee);border-radius:14px;padding:16px 8px;cursor:pointer;
                    display:flex;flex-direction:column;align-items:center;gap:4px;transition:all 0.2s ease;
                ">
                    <i class="fas fa-pen" style="font-size:18px;color:var(--accent-color, #c5a47e);"></i>
                    <span style="font-size:13px;font-weight:500;color:var(--text-primary, #1a1a1a);font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">自定义</span>
                </button>`;
            }
            const label = formatTimeOptionLabel(t);
            return `<button class="time-btn-dyn" data-time="${t}" style="
                background:var(--secondary-bg, #fff);border:1.5px solid var(--border-color, #eee);border-radius:14px;padding:16px 8px;cursor:pointer;
                display:flex;flex-direction:column;align-items:center;gap:4px;transition:all 0.2s ease;
            ">
                <span style="font-size:20px;font-weight:500;color:var(--accent-color, #c5a47e);line-height:1;font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">${label.main}</span>
                <span style="font-size:11px;color:var(--text-secondary, #888);font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">${label.unit}</span>
            </button>`;
        }).join('');

        const html = `
            <div style="display:flex;align-items:center;gap:8px;font-size:18px;font-weight:500;color:var(--text-primary, #1a1a1a);margin-bottom:10px;justify-content:center;font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;">
                <i class="fas ${cfg.icon}" style="color:var(--accent-color, #c5a47e);"></i>
                <span>${cfg.label}</span>
            </div>
            ...
            <div id="custom-time-panel" style="
                display:none;align-items:center;gap:10px;margin:-4px 0 16px;
                padding:10px;border:1px solid var(--border-color, rgba(0,0,0,0.08));border-radius:12px;
                background:var(--primary-bg, #fafafa);
            ">
                <input id="custom-time-input" type="number" min="1" max="1440" inputmode="numeric" placeholder="输入分钟" style="
                    flex:1;min-width:0;border:none;background:transparent;outline:none;
                    font-size:14px;color:var(--text-primary, #1a1a1a);
                    font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;
                ">
                <button id="custom-time-confirm" style="
                    border:none;border-radius:10px;padding:8px 14px;cursor:pointer;
                    background:var(--accent-color, #c5a47e);color:#fff;font-size:13px;
                    font-family:var(--font-family, 'Noto Serif SC', serif);letter-spacing:0;
                ">确定</button>
            </div>
        ...
        const customPanel = modal.querySelector('#custom-time-panel');
        const customInput = modal.querySelector('#custom-time-input');
        const customConfirm = modal.querySelector('#custom-time-confirm');
        const confirmCustomTime = () => {
            const minutes = normalizeCustomMinutes(customInput.value);
            if (!minutes) return;
        ...
        customConfirm.addEventListener('click', confirmCustomTime);
        customInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') confirmCustomTime();
        });
        ...
        modal.querySelectorAll('.time-btn-dyn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const selectedTime = applySelectedTime(t);
    ...
    function openCompanionPage(opts) {
        ...
        // 设置玻璃球计时器副标题（场景名英文缩写）
        const timerArea = $('companion-timer-area');
        if (timerArea) timerArea.dataset.mode = currentMode || '';

        const timerLabel = $('companion-timer-label');
        if (timerLabel) {
            const labels = { study: '学习中', work: '工作中', exercise: '运动中', sleep: '睡觉中' };
    ...
    function closeCompanionPage(opts) {
        ...
        const _diaryStartedAt = Date.now() - _diaryDurSec * 1000;
        ...
        try {
            if (_diaryMode && _diaryDurSec >= 30 && typeof window.addCompanionRecordToMood === 'function') {
                window.addCompanionRecordToMood({
                    id: `${_diaryStartedAt}-${_diaryMode}-${Math.round(_diaryDurSec)}`,
                    mode: _diaryMode,
                    modeLabel: MODES[_diaryMode]?.label || '陪伴',
                    duration: _diaryDurSec,
                    startedAt: _diaryStartedAt,
                    initiator: _diaryInitiator
                });
            }
        } catch (e) {
            console.warn('[companion] write mood companion record error:', e);
        }
    ...
    window._companionRecoverModule = {
        ...
        saveSessionAsDiary: async function(session) {
            ...
            try {
                if (typeof window.addCompanionRecordToMood === 'function') {
                    window.addCompanionRecordToMood({
                        id: `${session.startTs}-${session.mode}-${Math.round(duration)}`,
                        mode: session.mode,
                        modeLabel: MODES[session.mode]?.label || '陪伴',
                        duration: duration,
                        startedAt: session.startTs,
                        initiator: session.initiator || 'user'
                    });
                }
            } catch (e) {
                console.warn('[companion] write recovered mood record error:', e);
            }
        }
    };
    ...
    function showExtendInvitingByUser() {
        ...
        let timeText;
        if (!isCountdown) {
            timeText = getOpenEndedLabel(currentMode);
        } else {
            timeText = formatDurationText(Math.round(totalSeconds / 60));
        }
    ...
    function bindEvents() {
        // 顶部按钮 / 高级功能入口 —— 用事件委托，无论按钮何时出现都能响应
        document.addEventListener('click', function (e) {
            const btn = e.target.closest && e.target.closest('#companion-btn, #companion-function');
            if (btn) {
                e.stopPropagation();
                e.preventDefault();
                console.log('[companion] 陪伴按钮被点击');
                if (btn.id === 'companion-function') {
                    window.__companionReturnTarget = 'advanced-modal';
                    const advancedModal = document.getElementById('advanced-modal');
                    if (advancedModal) {
    ...
    window.companionModule = {
        ...
        testContextInvite: () => showIncomingCompanion(),
        pickContextualInviteMode,
        ...
        testEnter: async (mode = 'study', time = 10) => {
            ...
            applySelectedTime(time);
            openCompanionPage();
            console.log(`[companion] 已直接进入${mode}陪伴，${time === 'rest' ? getOpenEndedLabel(mode) : formatDurationText(parseInt(time, 10))}`);
        },
