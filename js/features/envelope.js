...
async function checkEnvelopeStatus() {
    ...
    envelopeData.outbox.forEach(letter => {
        if (letter.status === 'pending' && letter.replyTime && now >= letter.replyTime) {
...
const APPEARANCE_BACKGROUND_EXTRA_SECTIONS = [
    'companion-bg-section',
    'diary-bg-section',
    'companion-noise-section'
];
...
window.showAppearancePanel = function(panel) {
    ...
    const showBackgroundExtras = panel === 'font-bg' || panel === 'background';
    APPEARANCE_BACKGROUND_EXTRA_SECTIONS.forEach(id => {
        const section = document.getElementById(id);
        if (section) section.style.display = showBackgroundExtras ? 'block' : 'none';
    });
    if (panelMap[panel]) {
...
function setEnvTabBarVisible(visible) {
    const tabBar = document.querySelector('#envelope-modal .env-tab-bar');
    if (tabBar) tabBar.style.display = visible ? 'flex' : 'none';
}

window.switchEnvTab = function(tab) {
    currentEnvTab = tab;
    setEnvTabBarVisible(true);
...
function renderEnvelopeLists() {
    renderOutboxList();
    renderInboxList();
    const pendingCount = envelopeData.outbox.filter(l => l.status === 'sent' || l.status === 'received' || l.status === 'pending').length;
