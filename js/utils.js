...
function deduplicateContentArray(arr, baseSystemArray = []) {
    ...
    return { result, removedCount };
        ...
        // Expose this so invite overlays can stop looping sounds cleanly.
        ...
        const playSound = (type, loopOverride) => {
            ...
            // Companion invite sounds use dedicated files/settings.
            ...
            if (INVITE_TYPES.indexOf(type) !== -1) {
                try {
                    ...
                    const audio = new Audio(customUrl || inviteKey.defaultFile);
                    audio.volume = Math.min(1, Math.max(0, settings.soundVolume || 0.3));
                    audio.loop = (typeof loopOverride === 'undefined') ? true : !!loopOverride;
                    _currentAudio = audio;
                    audio.play().catch((e) => { console.warn('[playSound] invite audio play failed:', e); });
                    if (!audio.loop) audio.addEventListener('ended', () => { _currentAudio = null; });
                } catch (e) {
...
const APP_DEFAULT_FONT_FAMILY = "'Noto Serif SC', serif";

function resetToDefaultFontFamily() {
    document.documentElement.style.setProperty('--font-family', APP_DEFAULT_FONT_FAMILY);
    document.documentElement.style.setProperty('--message-font-family', APP_DEFAULT_FONT_FAMILY);
    if (typeof settings !== 'undefined') {
        settings.customFontUrl = "";
        settings.messageFontFamily = APP_DEFAULT_FONT_FAMILY;
    }
}

async function applyCustomFont(url) {
    if (!url || !url.trim()) {
        resetToDefaultFontFamily();
        return;
    }
