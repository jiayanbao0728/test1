function isMessageStatsEligible(msg) {
            if (!msg || !msg.text || !String(msg.text).trim()) return false;
            if (msg.sender === null || msg.sender === 'system') return false;
            const ignoredTypes = ['system', 'call-event', 'companion-event', 'companion-system', 'diary-event'];
            if (ignoredTypes.indexOf(msg.type) !== -1) return false;
            const text = String(msg.text).trim();
            return !/(邀请未接通|视频通话已结束|通话已结束|拒绝了这次|接受了.*邀请|发起了.*邀请)/.test(text);
        }
...
function renderStatsContent() {
            ...
            const chatMessages = partnerMessages.concat(myMessages)
            ...
            const allMsgs = chatMessages.filter(m => m.timestamp);
                ...
                <div class="stats-dashboard">
                    <div class="stats-overview-grid">
                        <div class="overview-item overview-large">
                            <div class="overview-value">${chatMessages.length}</div>
                    ...
                    <div class="stats-card">
                        ...
                        <div class="stats-card-title" id="stats-rank-title">
                            <i class="fas fa-user-circle"></i> 对方常说的话 TOP 5
                        </div>
        ...
        window.switchStatsView = function(who) {
            ...
            if (who === 'partner') {
                ...
                title.innerHTML = '<i class="fas fa-user-circle"></i> 对方常说的话 TOP 5';
                ...
                title.innerHTML = '<i class="fas fa-user"></i> 我方常说的话 TOP 5';
...
(function() {
    ...
    function tokenize(text) {
        text = text
            ...
            .toLowerCase();
        var words = {};
        var cnParts = text.match(/[\u4e00-\u9fa5]+/g) || [];

        function addWord(word, score) {
            if (!word || word.length < 2 || STOP_WORDS.has(word)) return;
            words[word] = (words[word] || 0) + score;
        }

        cnParts.forEach(function(part) {
            var len = part.length;
            var segments = [];
            if (window.Intl && Intl.Segmenter) {
                try {
                    var segmenter = new Intl.Segmenter('zh', { granularity: 'word' });
                    segments = Array.from(segmenter.segment(part))
                        .map(function(item) { return item.segment; })
                        .filter(function(word) { return word.length >= 2 && !STOP_WORDS.has(word); });
                } catch (e) {
                    segments = [];
                }
            }
            ...
            if (segments.length) {
                segments.forEach(function(word) {
                    addWord(word, 1 + Math.min(word.length, 4) * 0.18);
                });
                for (var s = 0; s + 1 < segments.length; s++) {
                    var phrase = segments[s] + segments[s + 1];
                    if (phrase.length >= 4 && phrase.length <= 8) addWord(phrase, 1.25);
                }
                return;
            }

            if (len >= 5 && len <= 8) addWord(part, 1.8);
            for (var small = 4; small >= 2; small--) {
                if (len < small) continue;
                for (var j = 0; j + small <= len; j += small) {
                    addWord(part.slice(j, j + small), small === 4 ? 1.8 : small === 3 ? 1.4 : 1);
                }
            }
        });

        // 英文和数字词独立统计，避免被中文规则截断。
        (text.match(/[a-z0-9]{3,20}/g) || []).forEach(function(w) {
            addWord(w, 1);
        });
        return words;
    }
    ...
    function topWords(freq, n) {
        ...
        return Object.entries(freq)
            .filter(function(e) {
                if (e[0].length >= 5 && e[1] < 2) return false;
                return e[1] >= min && e[0].length >= 2;
            })
            .sort(function(a, b) {
                var scoreA = a[1] * (1 + Math.min(a[0].length, 8) * 0.04);
                var scoreB = b[1] * (1 + Math.min(b[0].length, 8) * 0.04);
    ...
    window.renderWordCloud = function() {
        ...
        var partnerMsgs = messages.filter(function(m) { return isMessageStatsEligible(m) && m.sender !== 'user'; });
        var myMsgs      = messages.filter(function(m) { return isMessageStatsEligible(m) && m.sender === 'user'; });
